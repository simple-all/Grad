classdef RKForcedMotion < publicsim.funcs.movement.StateManager
    %RKFORCEDMOTION Simulates forced movement using RK4 integrator
    %   Simulates the movement of objects due to forces by integrating
    %   through time with an RK4 integrator.
    ...
    properties (Access = private)
        requiredFcns = {'getMass'};
    end
    
    methods
        
        function obj = RKForcedMotion(parent, calls, varargin)
        ...
        function [newState, inputOldState] = updateLocation(obj, inputOldState, timeOffset)
            ...
            while(totalTimeOffset > 0)
                ...
                netForce = obj.getForceAtTime(startTime, numel(oldState.acceleration));
            ...
            % Fill in any struct fields that were not used in integration
            allFields = fields(inputOldState);
            for i = 1:numel(allFields)
                if ~isfield(newState, allFields{i})
                    newState.(allFields{i}) = inputOldState.(allFields{i});
                end
            end
            
        end
        
        function netForce = getForceAtTime(obj, time, nDim)
            % Gets the net force on the parent object at the given time
            netForce = zeros(1, nDim);
            % Call each specified function to sum forces on the object for
            % the current time
            for i = 1:numel(obj.forceCalls)
                netForce = netForce + obj.parent.(obj.forceCalls{i})(time);
            end
        end
