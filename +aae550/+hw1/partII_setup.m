...
g7 = @(x) (x(1) - x(2)) / 0.02 - 1;
g8 = @(x) ((16 * (P + w * H) / (pi * (x(1)^4 - x(2)^4))) * (x(1)^2 + x(1) * x(2) + x(2)^2)) / tau_a - 1;
g9 = @(x) ((32 * (P * H + 0.5 * w * H^2) / (pi * (x(1)^4 - x(2)^4))) * x(1)) / sigma_a - 1;
g10 = @(x) ((64 / (pi * E * (x(1)^4 - x(2)^4))) * ((P * H^3) / 3 + (w * H^4) / 8)) / delta_a - 1;
